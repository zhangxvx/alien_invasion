...
def check_keydown_events(event, ai_settings, screen, stats, sb, play_button, ship, aliens, bullets):
    ...
    if event.key == pygame.K_RIGHT:
        ...
        sys.exit()
    elif event.key == pygame.K_x and not stats.game_active:
...
def save_high_score(stats):
    """保存最高分"""
        ...
        with open(filename,'w') as file_object:
            if stats.high_score > int(stats.contents):
                file_object.write(str(stats.high_score))
                # print("save " + str(stats.high_score))
            else:
                file_object.write(stats.contents)
